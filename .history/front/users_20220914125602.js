        ...
        .then((response) => {
            if (response.status === 200) {
                ...
                data.forEach(element => {
                    ...
                    trHTML += '<td><button type="button" class="btn btn-outline-secondary" onclick="showUserEditBox(' + element.id + ')"><img src="/images/iconEdit.png"></button>';</button>';
...
const listingUsers = () => {
    ...
    axios.get(`${ENDPOINT}/users?name=` + listingUsers)
        .then((response) => {
            if (response.status === 200) {
                const data = response.data;
                var trHTML = '';
