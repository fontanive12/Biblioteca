...
const listingUsers = () => {
    ...
    axios.get(`${ENDPOINT}/books?tile=` + listingUsers)
        .then((response) => {
            if (response.status === 200) {
                const data = response.data;
                var trHTML = '';
...
//         if (params.Category.description) {
//             where.Category.description = {
//                 [Op.iLike]: `%${params.Category.description}%`
//             };
//         }

//         const listing = await Book.findAll({
//             where: where
//         });
//         res.json(listing);
//     }
