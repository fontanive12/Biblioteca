        ...
        .then((response) => {
            if (response.status === 200) {
                ...
                data.forEach(element => {
                    ...
                    trHTML += '<td><button type="button" class="btn btn-outline-secondary" onclick="showPublisherEditBox(' + element.id + ')"><img src="/images/edit.png"></button>';
                    trHTML += '<button type="button" class="btn btn-outline-danger" onclick="publisherDelete(' + element.id + ')"><img src="/images/delete.png"></button></td>';
                    trHTML += "</tr>";
                });
...
const listingPublisher = () => {
    const input1 = document.querySelector("#listingPublisher");
    const listingPublisher = input1.value;

    axios.get(`${ENDPOINT}/publishers?name=` + listingPublisher)
        .then((response) => {
            if (response.status === 200) {
                const data = response.data;
                var trHTML = '';
