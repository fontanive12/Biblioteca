        ...
        .then((response) => {
            if (response.status === 200) {
                ...
                data.forEach(element => {
                    ...
                    trHTML += '<td><button type="button" class="btn btn-outline-secondary" onclick="showBookEditBox(' + element.id + ')"><img src="/images/iconEdit.png"></button>';
                    trHTML += '<button type="button" class="btn btn-outline-danger" onclick="bookDelete(' + element.id + ')"><img src="/images/delete.png"></button></td>';
                    trHTML += "</tr>";
                });
                document.getElementById("mytable").innerHTML = trHTML;
...
const listingTitle = () => {
    ...
    const category = document.getElementById("loadCategories").value;
    // console.log(category);
    const listingTitle = input1.value;

    axios.get(`${ENDPOINT}/books?title=${listingTitle}&category=${category}`)
        .then((response) => {
            if (response.status === 200) {
                const data = response.data;
                var trHTML = '';
