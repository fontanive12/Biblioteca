...
const listingName= () => {
    const input1 = document.querySelector("#listingName");
    const category = document.getElementById("listingName").value;
    // console.log(category);
    const listingName = input1.value;

    axios.get(`${ENDPOINT}/books?name=${listingName}`)
        .then((response) => {
            if (response.status === 200) {
                const data = response.data;
                var trHTML = '';
